...
def iter_test_shfl_record(model, dataset, lines, batch_size):
    batch = {'query_id': [], 'doc_id': [], 'query_tok': [], 'doc_tok': []}
    for qid, did, query_tok, doc_tok in _iter_test_shfl_record(model, dataset, lines):
        batch['query_id'].append(qid)
        batch['doc_id'].append(did)
        batch['query_tok'].append(query_tok)
        batch['doc_tok'].append(doc_tok)
...
def _iter_test_shfl_record(model, dataset, lines):
    ds_queries, ds_docs = dataset
    for line in lines:
